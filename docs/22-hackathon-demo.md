...
- **Update (Simulation page's bridge-observer pane):** `GET /simulation/live` — which reflects `simulation/sumo_live_bridge.py`'s real corridor simulation, as distinct from the sandbox above — genuinely depends on Redis pub/sub for the bridge's ticks and a Redis `GET` for its heartbeat key. Under this same drill, that pane honestly flips to `unavailable` (no tick can arrive without pub/sub, and the heartbeat read fails), while the sandbox's own `/simulation/start`/`/state`/`/step` behavior above is unaffected. This is correct, not a regression: the bridge pane's whole purpose is to say "no data" rather than hold a stale reading, and Redis genuinely is that pane's only path to the bridge's data.
