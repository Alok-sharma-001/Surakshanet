...
def test_simulation_start_returns_non_blocking_response(authed_client: E2EHttpClient):
...
@pytest.mark.m3
@pytest.mark.feature(12)
def test_simulation_status_queryable(authed_client: E2EHttpClient):
    """TC-F12-02: Verify GET /api/v1/simulation/status returns current simulation state."""
    res = authed_client.get("/api/v1/simulation/status")
...
@pytest.mark.m3
@pytest.mark.feature(12)
def test_simulation_stop_endpoint(authed_client: E2EHttpClient):
    """TC-F12-03: Verify POST /api/v1/simulation/stop halts running background task."""
    res = authed_client.post("/api/v1/simulation/stop")
...
@pytest.mark.m3
@pytest.mark.feature(12)
def test_marl_training_task_decoupled(authed_client: E2EHttpClient):
...
@pytest.mark.m3
@pytest.mark.feature(12)
def test_concurrent_api_health_during_background_work(authed_client: E2EHttpClient):
    """TC-F12-05: Verify /api/v1/health continues responding <200ms during background tasks."""
    res = authed_client.get("/api/v1/health")
